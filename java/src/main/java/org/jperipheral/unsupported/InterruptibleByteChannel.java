...
public interface InterruptibleByteChannel
{
	 ...
	 * channel into the given buffer. The {@code
	 * handler} parameter is a completion handler that is invoked when the read operation completes
	 * (or fails). The result passed to the completion handler is the number of bytes read or {@code -1}
	 ...
	 * that bytes have not been read, or will not be read from the channel into the given buffer, then
	 * further attempts to read from the channel will cause an unspecific runtime exception to be
	 * thrown.
	 *
	 * <p> Otherwise this method works in the same manner as the {@link
	 * AsynchronousByteChannel#read(java.nio.ByteBuffer, java.lang.Object, java.nio.channels.CompletionHandler)}
	 * method.
	 *
	 ...
	 * @throws ShutdownChannelGroupException If the channel is associated with a {@link AsynchronousChannelGroup group}
	 * that has terminated
